...
function inferProjectFromAbsolutePath(
  ...
  if (workspaceRoot) {
    return { projectDir: workspaceRoot, source: "workspace_root", confidence: 0.98 };
  }

  // 2) Prefer stable known roots from session context.
  if (normalizedInput && isPrefixPath(absPath, normalizedInput)) {
    return { projectDir: normalizedInput, source: "input_cwd", confidence: 0.88 };
  }
  if (normalizedOrigin && isPrefixPath(absPath, normalizedOrigin)) {
    return { projectDir: normalizedOrigin, source: "session_origin", confidence: 0.82 };
  }
  if (normalizedLast && isPrefixPath(absPath, normalizedLast)) {
    return { projectDir: normalizedLast, source: "last_seen", confidence: 0.76 };
  }

  // 3) Direct cwd events indicate explicit operator intent to shift project.
  if (event.type === "cwd") {
    return { projectDir: absPath, source: "cwd_event", confidence: 0.9 };
  }
  ...
  return { projectDir, source: "event_path", confidence: 0.7 };
}
...
export function resolveProjectAttributions(
  ...
  for (const ev of events) {
    ...
    if (attribution.projectDir && attribution.confidence >= 0.55) {
