...
| `CTX_ROUTING_SKIP_AGENTS` | _(unset)_ | Override the per-subagent routing-block skip-list ([#641](https://github.com/mksglu/context-mode/issues/641)). By default the routing block is **skipped** when spawning subagents whose `subagent_type` matches a baked list of agents verified to lack `ctx_*` MCP tools — Claude Code built-ins (`Explore`, `Plan`) plus plugin agents whose frontmatter declares an explicit `tools:` whitelist without `ctx_*` (currently `caveman:*`, `feature-dev:*` — see [`hooks/core/routing.mjs`](hooks/core/routing.mjs)). Plugin agents without a `tools:` field inherit parent tools (including `ctx_*`) and are NOT skipped by default. Syntax: comma-separated tokens; bare or `+`-prefixed names ADD to the skip-list, `-`-prefixed names REMOVE. Examples — `MyAgent` (skip MyAgent), `-Explore` (opt Explore back in to receive the block), `foo,-Plan,bar:*` (add foo + bar:* wildcard, remove Plan). Bash → general-purpose conversion is unaffected and always fires. |
