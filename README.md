...
| `CTX_ROUTING_SKIP_AGENTS` | _(unset)_ | Override the per-subagent routing-block skip-list ([#641](https://github.com/mksglu/context-mode/issues/641)). By default the ~580-token routing block is **skipped** when spawning subagents whose `subagent_type` matches a baked list of agents known to lack `ctx_*` MCP tools (`Explore`, `Plan`, plus plugin-namespaced wildcards like `caveman:*`, `pr-review-toolkit:*`, `feature-dev:*` — see [`hooks/core/routing.mjs`](hooks/core/routing.mjs)). Syntax: comma-separated tokens; bare or `+`-prefixed names ADD to the skip-list, `-`-prefixed names REMOVE. Examples — `MyAgent` (skip MyAgent), `-Explore` (opt Explore back in to receive the block), `foo,-Plan,bar:*` (add foo + bar:* wildcard, remove Plan). Bash → general-purpose conversion is unaffected and always fires. |
